...
//
// Tries CPAL speaker/loopback capture first; falls back to synthetic PCM if unavailable.
// On Windows, WASAPI allows opening the output device as an input stream (loopback).
//
// Usage:
//   cargo run                  # Speaker loopback (press Ctrl+C to stop)
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // println!("===========================================================");
    // println!("   Foundry Local -- Live Speaker Transcription (Rust)");
    // println!("===========================================================");
    // println!();
    ...
    // Try CPAL loopback (WASAPI output device as input); fall back to synthetic PCM on failure.

    let mut mic_active = false;

    if !use_synth {
        match try_start_loopback(&session, &running).await {
            Ok(()) => {
                mic_active = true;
            }
            Err(e) => {
                eprintln!("Could not initialize speaker loopback: {e}");
...
/// Open the default output device as a loopback input stream (WASAPI on Windows)
/// and forward PCM to the session. Blocks until Ctrl+C is pressed.
async fn try_start_loopback(
    session: &Arc<LiveAudioTranscriptionSession>,
    running: &Arc<AtomicBool>,
) -> Result<(), Box<dyn std::error::Error>> {
    let host = cpal::default_host();
    // On Windows/WASAPI, default_output_device() opened as an input stream gives loopback.
    let device = host
    ...
    // println!("  Capturing speaker/system audio (loopback).");
